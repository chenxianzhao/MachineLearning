...
class perceptron:
    ...
    def getdeltaomega(self):
        ...
        for h in range(self.hidden_size):
            for j in range(self.output_size):
                self.deltaomega[h][j] += self.learnrate*self.g[j]*self.hiddenoutput[h];
    ...
    def getdeltanu(self):
        ...
        for i in range(self.input_size):
            for h in range(self.hidden_size):
                self.deltanu[i][h] += self.learnrate*self.e[h]*self.input[i];
    ...
    def reset(self):
        ...
        self.deltaomega = np.zeros((self.hidden_size, self.output_size));
        self.deltatheta = np.zeros(self.output_size);
        self.deltanu = np.zeros((self.input_size, self.hidden_size));
        self.deltagamma = np.zeros(self.hidden_size);

    def updatetogether(self):
        self.h2o_Mat += self.deltaomega;
        self.outputbase += self.deltatheta;
        self.i2h_Mat += self.deltanu;
        self.hiddenbase += self.deltagamma;
